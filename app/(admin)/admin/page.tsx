...
// Import extracted components & data
...
function NavItem({ icon: Icon, label, active, onClick, collapsed }: NavItemProps) {
  return (
    ...
    >
      {active && (
        <div className="absolute left-0 top-1/2 -translate-y-1/2 h-6 w-[3px] bg-[#E91E63] rounded-r-md -ml-2" />
      )}
      <Icon
        size={18}
        className={active ? "text-[#E91E63]" : "group-hover:text-gray-900 transition-colors"}
      />
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#FEF8F9] font-sans text-gray-900">
      ...
      <div className="md:hidden hidden fixed inset-0 bg-black/50 z-40" />

      {/* Sidebar Navigation */}
      <aside
        className={`${isSidebarCollapsed ? "w-[80px]" : "w-[260px]"
          } bg-[#FEF8F9] flex flex-col h-screen sticky top-0 transition-all duration-300 z-50 shrink-0 border-r border-[#F3E8EC]`}
      >
        <div className="h-28 flex items-center justify-center px-6 shrink-0 relative">
          {!isSidebarCollapsed && (
            <Image src="/logo.png" alt="Inventino" width={160} height={60} className="object-contain" priority />
          )}
          {isSidebarCollapsed && (
            <span className="text-2xl font-black italic tracking-tight uppercase text-[#E91E63]">
              I<span className="text-gray-300">.</span>
            </span>
          )}
          ...
          <button
            ...
            className="absolute -right-3.5 top-1/2 -translate-y-1/2 bg-white border border-gray-200 rounded-full p-1 text-gray-400 hover:text-[#E91E63] shadow-[0_2px_8px_-2px_rgba(0,0,0,0.05)] transition-all hidden md:flex z-50"
          >
        ...
        <div className="flex-1 overflow-y-auto py-2 pb-8 space-y-7 scrollbar-hide">
          {navigationGroups.map((group, idx) => (
            <div key={idx} className="space-y-1">
              {!isSidebarCollapsed && (
                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest px-8 mb-3">
                  {group.title}
                </p>
              )}
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 bg-white rounded-full p-1 pr-4 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] cursor-pointer hover:bg-gray-50 transition-colors">
              ...
              <div className="hidden sm:flex flex-col justify-center">
                <p className="text-[12px] font-bold text-gray-900 leading-none mb-1 shadow-transparent">Admin User</p>
