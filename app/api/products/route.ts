...
export async function GET(request: NextRequest) {
  ...
  let filteredProducts = products.map(p => ({
    ...
    price: p.price,
    category: p.category.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase()), // e.g., "hair-accessories" -> "Hair Accessories"
  ...
  // Filter by type
  if (type !== 'all') {
    filteredProducts = filteredProducts.filter(p => p.category.toLowerCase() === type.toLowerCase());
  }
