...
interface ToggleProps { enabled: boolean; onChange: (v: boolean) => void; accentColor: string; }
function Toggle({ enabled, onChange, accentColor }: ToggleProps) {
  return (
    <button role="switch" aria-checked={enabled} onClick={() => onChange(!enabled)}
      style={{ width: 46, height: 24, borderRadius: 12, background: enabled ? accentColor : "#E5E7EB",
        border: "none", cursor: "pointer", position: "relative", flexShrink: 0,
        transition: "background 0.22s", padding: 0 }}>
      <span style={{ position: "absolute", top: 3, left: enabled ? 23 : 3, width: 18, height: 18,
        borderRadius: "50%", background: "white", boxShadow: "0 1px 4px rgba(0,0,0,0.18)",
...
interface PasswordFieldProps {
  ...
  showPwd: boolean; toggleShow: () => void;
  error?: string; hint?: string;
}
function PasswordField({ label, value, onChange, showPwd, toggleShow, error, hint }: PasswordFieldProps) {
  return (
    <div>
      <label className="form-label">{label} <span className="form-label-req">*</span></label>
      <div style={{ position: "relative" }}>
        <input className={`input-basic${error ? " input-error" : ""}`}
          type={showPwd ? "text" : "password"} value={value} onChange={onChange}
...
function SuccessBanner({ message }: { message: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#f0fdf4",
      border: "1px solid #bbf7d0", borderRadius: 8, padding: "10px 14px", marginTop: 16 }}>
      <Icons.Check size={14} />
      <span style={{ fontSize: 13, color: "#16a34a", fontWeight: 500, fontFamily: "'Roboto', sans-serif" }}>{message}</span>
    </div>
  );
}
...
function Card({ children, style = {} }: { children: React.ReactNode; style?: React.CSSProperties }) {
  return (
    <div style={{ background: "white", borderRadius: 14, padding: "22px 22px", marginBottom: 18,
      boxShadow: "0 1px 10px rgba(217,79,122,0.06)", border: "1px solid #fce8f0", ...style }}>
      {children}
    </div>
  );
}
...
function SectionTitle({ icon: Icon, children }: { icon?: React.ComponentType<IconProps>; children: React.ReactNode }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 15, fontWeight: 700,
      color: "#111827", marginBottom: 18, fontFamily: "'Roboto', sans-serif" }}>
      ...
      <Card style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        <div style={{ position: "relative", flexShrink: 0 }}>
          <div style={{ width: 76, height: 76, borderRadius: "50%",
            ...
            border: "3px solid white", boxShadow: "0 4px 16px rgba(217,79,122,0.3)" }}>
            {avatarInitial}
          </div>
          <button style={{ position: "absolute", bottom: 0, right: 0, width: 26, height: 26,
            borderRadius: "50%", background: "#D94F7A", border: "2px solid white",
            display: "flex", alignItems: "center", justifyContent: "center",
        ...
        <div>
          ...
          <span style={{ fontSize: 11, fontWeight: 600, color: "#D94F7A",
            background: "#D94F7A15", padding: "3px 10px", borderRadius: 20 }}>
            Premium Member
          </span>
        </div>
      </Card>
      ...
      <Card>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(210px, 1fr))", gap: 16 }}>
          <div>
            ...
            <input className="input-basic" value={displayName} placeholder="Your full name"
              onChange={(e) => dispatch(setAccountField({ name: e.target.value }))} />
          </div>
          <div>
            <label className="form-label">Email Address <span className="form-label-req">*</span></label>
            <input className="input-basic" type="email" value={displayEmail} placeholder="you@example.com"
              onChange={(e) => dispatch(setAccountField({ email: e.target.value }))} />
            <p className="form-hint">Used for login and order updates</p>
          </div>
          <div>
            <label className="form-label">Phone Number <span className="form-label-opt">(optional)</span></label>
            <input className="input-basic" type="tel" value={displayPhone} placeholder="+91 00000 00000"
              onChange={(e) => dispatch(setAccountField({ phone: e.target.value }))} />
          </div>
          <div>
            <label className="form-label">Location <span className="form-label-opt">(optional)</span></label>
            <input className="input-basic" value={displayLocation} placeholder="City, Country"
              onChange={(e) => dispatch(setAccountField({ location: e.target.value }))} />
          </div>
          <div>
            <label className="form-label">Gender <span className="form-label-opt">(optional)</span></label>
            <div className="select-wrapper">
              <select className={`input-select${!account.gender ? " placeholder-active" : ""}`}
                value={account.gender}
                onChange={(e) => dispatch(setAccountField({ gender: e.target.value }))}>
              ...
              <span className="select-chevron"><Icons.ChevronDown size={13} /></span>
            </div>
          </div>
        </div>
        <div style={{ marginTop: 22 }}>
          <button className="save-btn" style={{ display: "inline-flex", alignItems: "center",
            background: "#D94F7A", color: "white", border: "none", borderRadius: 8,
            padding: "0 20px", height: 44, fontSize: 13.5, fontWeight: 600,
            cursor: "pointer", fontFamily: "'Roboto', sans-serif" }} onClick={save}>
            Update Account
          </button>
        </div>
        {account.savedBanner && <SuccessBanner message="Account updated successfully!" />}
      </Card>
    </div>
  );
}
  ...
  return (
    <div>
      <Card>
        ...
        <p className="form-hint" style={{ marginTop: -10, marginBottom: 16 }}>Choose what updates you'd like to receive.</p>
        <div>
          {items.map((item) => (
            <div key={item.id} className="notif-row">
              ...
              <Toggle accentColor={accentColor} enabled={item.enabled}
      ...
      <Card style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap", background: "#fff5f8", border: "1px solid #f5c6d8" }}>
        ...
        <button style={{ display: "inline-flex", alignItems: "center", background: "white",
          color: accentColor, border: `1.5px solid ${accentColor}`, borderRadius: 8,
          padding: "0 20px", height: 44, fontSize: 13.5, fontWeight: 600,
...
function SecurityTab({ accentColor }: { accentColor: string }) {
  ...
  const pwdStrength = (() => {
    const p = security.newPwd; if (!p) return 0;
    let s = 0;
    if (p.length >= 8)           s++;
    if (/[A-Z]/.test(p))         s++;
    if (/[0-9]/.test(p))         s++;
  ...
  const btnStyle: React.CSSProperties = {
    display: "inline-flex", alignItems: "center", background: accentColor,
    color: "white", border: "none", borderRadius: 8, padding: "0 20px", height: 44,
    fontSize: 13.5, fontWeight: 600, cursor: "pointer", fontFamily: "'Roboto', sans-serif",
  };
    ...
    <div>
      <Card>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <PasswordField label="Current Password" value={security.current}
            onChange={(e) => dispatch(setSecurityField({ current: e.target.value }))}
            showPwd={showCurrent} toggleShow={() => setShowCurrent((v) => !v)}
            error={security.errors.current} />

          <PasswordField label="New Password" value={security.newPwd}
            onChange={(e) => dispatch(setSecurityField({ newPwd: e.target.value }))}
            showPwd={showNew} toggleShow={() => setShowNew((v) => !v)}
          ...
          <PasswordField label="Confirm New Password" value={security.confirm}
            onChange={(e) => dispatch(setSecurityField({ confirm: e.target.value }))}
            showPwd={showConfirm} toggleShow={() => setShowConfirm((v) => !v)}
            error={security.errors.confirm} />
        </div>
        {security.savedBanner && <SuccessBanner message="Password changed successfully!" />}
        <div style={{ marginTop: 20 }}>
          <button className="save-btn" style={btnStyle} onClick={handleSubmit}>Change Password</button>
        </div>
      </Card>
      ...
      <Card>
        ...
        {[
          ...
          <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
            padding: "13px 0", borderTop: i > 0 ? "1px solid #fce8f0" : "none",
            gap: 12, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 40, height: 40, borderRadius: 10, background: "#fff5f8",
                border: "1px solid #fce8f0", display: "flex", alignItems: "center",
                justifyContent: "center", color: "#D94F7A" }}>
            ...
            {s.current ? (
              <span style={{ fontSize: 12, fontWeight: 600, color: "#22c55e", background: "#f0fdf4",
                padding: "4px 12px", borderRadius: 20, border: "1px solid #bbf7d0" }}>● Active Now</span>
            ) : (
    ...
    <div>
      <Card>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {([
            ...
            <button key={id} className={`theme-card${theme === id ? " tc-active" : ""}`}
      ...
      <Card>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
          {accentOptions.map(({ color, label }) => (
            <button key={color} title={label} onClick={() => dispatch(setAccentColor(color))}
              style={{ width: 40, height: 40, borderRadius: "50%", background: color,
                ...
                boxShadow: accentColor === color ? `0 4px 12px ${color}55` : "none",
      ...
      <Card>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
          ...
          <Toggle accentColor={accentColor} enabled={reducedMotion}
...
// ── Root settings page ────────────────────────────────────────────────────
interface TabDef { id: string; label: string; Icon: React.ComponentType<IconProps>; }
  ...
  return (
    <>
      ...
      <div style={{ minHeight: "100vh", background: "linear-gradient(145deg, #fff5f8 0%, #f5f0ff 100%)", fontFamily: "'Roboto', sans-serif" }}>
        ...
        <div className="s-mobile-topbar" style={{ alignItems: "center",
          justifyContent: "space-between", padding: "13px 16px", background: "white",
          borderBottom: "1px solid #fce8f0", boxShadow: "0 1px 8px rgba(217,79,122,0.07)",
          position: "sticky", top: 0, zIndex: 40 }}>
          <span style={{ fontSize: 17, fontWeight: 700, color: accentColor, fontFamily: "'Roboto', sans-serif" }}>Settings</span>
          <button onClick={() => setSidebarOpen((v) => !v)}
            style={{ background: "#fff0f5", border: "none", borderRadius: 8, padding: "7px 9px",
              cursor: "pointer", color: accentColor, display: "flex" }}>
        ...
        <div className={`s-mobile-overlay${sidebarOpen ? " open" : ""}`}>
          <p style={{ fontSize: 11, fontWeight: 700, color: "#9ca3af", textTransform: "uppercase",
            letterSpacing: "0.08em", marginBottom: 12 }}>Menu</p>
          {tabs.map(({ id, label, Icon }) => (
            <button key={id} className={`s-tab${activeTab === id ? " s-tab-active" : ""}`}
        ...
        <div className="s-root" style={{ display: "flex", maxWidth: 1060, margin: "0 auto",
          padding: "30px 20px", gap: 20, alignItems: "flex-start" }}>

          {/* ✅ Desktop sidebar — single sidebar, hidden on mobile via CSS */}
          <div className="s-sidebar">
            <p style={{ fontSize: 11, fontWeight: 700, color: "#9ca3af", textTransform: "uppercase",
              letterSpacing: "0.08em", paddingBottom: 14, marginBottom: 8,
              borderBottom: "1px solid #fce8f0" }}>Menu</p>
            {tabs.map(({ id, label, Icon }) => (
              <button key={id} className={`s-tab${activeTab === id ? " s-tab-active" : ""}`}
                onClick={() => selectTab(id)}>
                <Icon size={16} />{label}
