...
const RECENT_ACTIVITY = [
  ...
  { text: "New product added", time: "3 hours ago", icon: PlusCircle, color: "text-pink-500", bg: "bg-pink-100" },
]; //
...
export default function AdminDashboard() {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 font-sans text-gray-800">
      ...
      {/* --- SIDEBAR --- */}
      <aside className="w-64 bg-white border-r border-gray-100 fixed left-0 top-16 h-[calc(100vh-4rem)] z-30 flex flex-col font-sans shadow-sm">
        <div className="p-6 flex-shrink-0">
          <h1 className="text-3xl font-serif text-[#D94F7A]">Inventino</h1>
          <p className="text-[10px] text-gray-400 tracking-widest uppercase mt-1">A la mode handcrafted</p>
        </div>

        <nav className="flex-1 px-4 overflow-y-auto pb-10 scrollbar-thin scrollbar-thumb-gray-200 scrollbar-track-transparent">
          <p className="px-4 text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 mt-2">Main</p>
          <NavItem icon={LayoutDashboard} label="Dashboard" active={activeTab === "Dashboard"} onClick={() => setActiveTab("Dashboard")} />
          <NavItem icon={ShoppingCart} label="Orders" active={activeTab === "Orders"} onClick={() => setActiveTab("Orders")} />

          <p className="px-4 text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 mt-6">Products</p>
          ...
          <p className="px-4 text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 mt-6">Analytics</p>
          ...
          <p className="px-4 text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 mt-6">Other</p>
      ...
      {/* --- MAIN CONTENT --- */}
      <main className="flex-1 ml-64 p-8 bg-[#FDF8F9]"> 
        <header className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <input type="text" placeholder="Search orders, products..." className="pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-full text-sm focus:outline-none focus:border-[#D94F7A] w-64 shadow-sm" />
            </div>
            <button className="p-2 bg-white border border-gray-200 rounded-full text-gray-500 hover:text-[#D94F7A] shadow-sm relative">
              <Bell size={20} />
              <span className="absolute top-0 right-0 w-2.5 h-2.5 bg-red-500 border-2 border-white rounded-full"></span>
            </button>
            <div className="flex items-center gap-3 pl-4 border-l border-gray-200">
              <div className="w-9 h-9 bg-[#D94F7A] rounded-full flex items-center justify-center text-white font-bold">A</div>
...
// --- DASHBOARD VIEW WITH RECENT ORDERS (FIGMA DESIGN) ---
function DashboardView() {
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex gap-2">
              <button className="px-3 py-1 text-xs font-medium text-gray-500 hover:text-[#D94F7A]">7 Days</button>
              <button className="px-3 py-1 text-xs font-medium bg-[#D94F7A] text-white rounded-lg">30 Days</button>
              <button className="px-3 py-1 text-xs font-medium text-gray-500 hover:text-[#D94F7A]">90 Days</button>
            </div>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          ...
          <div className="space-y-6">
            <CategoryProgress label="Jewelry" percent={45} color="bg-[#D94F7A]" />
      ...
      {/* --- RECENT ORDERS TABLE (RESTORED TO ORIGINAL FIGMA STYLE) --- */}
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 mb-8 overflow-hidden">
        <div className="p-6 border-b border-gray-50 flex justify-between items-center">
          <h3 className="font-bold text-gray-800">Recent Orders</h3>
          <button className="text-xs font-bold text-[#D94F7A] hover:underline">View All &rarr;</button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-gray-600">
            <thead className="bg-gray-50 text-gray-400 font-medium text-xs uppercase tracking-wider">
              <tr>
             ...
             {TOP_PRODUCTS.map((product, idx) => (
                <div key={idx} className="flex items-center gap-4 p-3 hover:bg-gray-50 rounded-xl transition-colors">
                  ...
                  <div className="text-right">
                    <p className="font-bold text-[#D94F7A]">{product.sales}</p>
...
function NavItem({ icon: Icon, label, active, onClick }: { icon: any, label: string, active: boolean, onClick: () => void }) {
  return (
    <button onClick={onClick} className={`flex items-center gap-3 w-full px-4 py-2.5 rounded-xl text-sm font-medium transition-all mb-1 ${active ? "bg-[#D94F7A] text-white shadow-md shadow-pink-200" : "text-gray-500 hover:bg-pink-50 hover:text-[#D94F7A]"}`}>
      <Icon size={18} />
      {label}
    </button>
  );
