...
export default function ProductDetailsPage() {
  ...
  useEffect(() => {
    if (mainImageScrollRef.current) {
      ...
      if (!isMobile) {
        container.scrollTo({
          left: selectedImage * container.offsetWidth,
  ...
  const isSaved = (savedItems as any).some(
  ...
  return (
    <div className="bg-white font-sans overflow-x-hidden min-h-screen relative pb-4 md:pb-0">
      {/* Toast Notification */}
      <div
        className={`fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] transition-all duration-700 ease-out ${isAdded ? "opacity-100 translate-y-0 scale-100 animate-float" : "opacity-0 translate-y-12 scale-95 pointer-events-none"}`}
      >
        <Link
          href="/bag"
          className="bg-[#1A1A1A]/95 backdrop-blur-md border border-white/10 text-white px-8 py-3.5 rounded-full shadow-2xl flex items-center gap-4 font-bold hover:bg-black transition-all group"
        >
      ...
      <div className="max-w-7xl mx-auto px-4 pt-6">
        <Link
          href="/products"
          className="inline-flex items-center text-gray-500 hover:text-[#D94F7A] transition-colors gap-2 text-sm font-bold"
        >
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-2 gap-12 border-b border-gray-100">
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-4">
            ...
            >
              ...
              <div className="absolute top-6 left-6 flex flex-col gap-2 z-30 pointer-events-none">
                <div className="bg-[#E05C7E] text-white text-[13px] font-bold px-5 py-2 rounded-full shadow-md">
                  Bestseller
                </div>
                <div className="bg-[#4CAF50] text-white text-[13px] font-bold px-5 py-2 rounded-full shadow-md">
                  New
                </div>
              </div>
              ...
              <div className="absolute top-4 right-4 flex flex-col gap-3 z-20">
                <button
                  onClick={() => handleSaved(product as any)}
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            <div
              ...
              className="flex gap-4 overflow-x-auto no-scrollbar snap-x max-w-fit"
            >
              {product.images.map((img, idx) => (
                <button
                  key={idx}
                  onClick={() => setSelectedImage(idx)}
                  className={`shrink-0 w-16 h-16 md:w-24 md:h-24 rounded-2xl overflow-hidden border-2 transition-all snap-center ${selectedImage === idx ? "border-[#D94F7A] ring-4 ring-[#D94F7A]/10 scale-95" : "border-gray-100 hover:border-gray-300"}`}
                >
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-center gap-3">
            <span className="text-3xl font-bold text-[#D94F7A]">
          ...
          <p className="text-gray-600 leading-relaxed text-sm border-b border-gray-100 pb-8">
            {product.description}
          </p>
          ...
          <div className="flex gap-4 pt-4">
            <div className="flex items-center bg-[#FFF1F2] rounded-full h-14 px-2 border border-pink-50 shadow-sm p-1">
              <button
                ...
                className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-[#D94F7A] transition-all hover:scale-110"
              >
            ...
            <button
              ...
              disabled={isAdded}
              className={`flex-1 font-bold rounded-2xl shadow-xl flex items-center justify-center gap-3 h-14 transition-all ${isAdded ? "bg-emerald-600 text-white shadow-emerald-100" : "bg-[#D94F7A] hover:bg-[#b83d63] text-white"}`}
            >
              {isAdded ? (
                <>
          ...
          <div className="mt-8 border border-gray-200 rounded-2xl overflow-hidden">
              ...
              >
                <div className="text-left pr-4 flex flex-col items-start">
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className="bg-[#D94F7A] text-white text-[10px] font-bold px-2.5 py-1 rounded-full uppercase inline-flex items-center">
                ...
                <div
                  className={`p-2 bg-gray-100 rounded-full transition-transform duration-300 ${activeAccordion === "reviews" ? "rotate-180" : ""}`}
                >
                ...
                <div className="p-6 pt-0 animate-in fade-in slide-in-from-top-2 flex flex-col gap-6">
                  <div className="border-b border-gray-50 pb-4">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-8 h-8 rounded-full bg-pink-100 flex items-center justify-center text-[#D94F7A] font-bold text-xs">
                        S
                      </div>
                      <div>
                  ...
                  <button
                    ...
                    className="text-[#D94F7A] text-xs font-bold uppercase tracking-wider hover:underline text-center"
                  >
            ...
            <div>
              <button
                onClick={() =>
                  setActiveAccordion(
                    activeAccordion === "detail" ? null : "detail",
                  )
                }
                className="w-full flex items-center justify-between p-6 hover:bg-gray-50/50 transition-colors"
              >
                ...
                <div
                  className={`p-2 bg-gray-100 rounded-full transition-transform duration-300 ${activeAccordion === "detail" ? "rotate-180" : ""}`}
                >
                  <ChevronDown size={20} className="text-gray-600" />
                </div>
              </button>
              {activeAccordion === "detail" && (
                <div className="p-6 pt-0 text-sm text-gray-600 animate-in fade-in slide-in-from-top-2 leading-relaxed">
                  Crafted with precision using ethically sourced materials. This
                  piece features a unique artisan design, perfect for both daily
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-10">
          <div className="w-full md:flex-1 relative aspect-video rounded-3xl overflow-hidden shadow-2xl group/story">
            ...
            <div className="absolute bottom-6 left-6 bg-white/95 backdrop-blur-sm rounded-full py-2.5 px-5 flex items-center gap-3 shadow-2xl z-20">
              <div className="w-9 h-9 rounded-full bg-[#D94F7A] flex items-center justify-center text-white font-bold text-xs">
                SA
              </div>
          ...
          <div className="w-full md:flex-1 bg-[#0F0F0F] rounded-3xl p-8 md:p-14 relative border border-white/5 shadow-[0_20px_50px_rgba(0,0,0,0.5)] z-10">
            ...
            <div className="pl-6 md:pl-10 space-y-6">
              <p className="text-white text-lg md:text-2xl italic font-medium leading-relaxed">
                "Every piece I create is infused with love and intention. I want
                the wearer to feel special and confident, knowing they're
                wearing something truly unique."
              </p>
              <div className="flex items-center gap-3">
                <span className="w-8 h-0.5 bg-[#D94F7A]"></span>
                <p className="text-[#D94F7A] text-xs font-bold uppercase tracking-widest">
                  Master Artisan
                </p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 md:py-20 relative group/similar">
        ...
        <button
          ...
          className="absolute left-0 top-1/2 -translate-y-1/2 z-30 p-3 bg-white/90 backdrop-blur-md rounded-full shadow-xl opacity-0 group-hover/similar:opacity-100 transition-all -translate-x-4 group-hover/similar:translate-x-2"
        >
          <ChevronLeft size={24} />
        </button>
        <button
          onClick={() => scrollSimilar("right")}
          className="absolute right-0 top-1/2 -translate-y-1/2 z-30 p-3 bg-white/90 backdrop-blur-md rounded-full shadow-xl opacity-0 group-hover/similar:opacity-100 transition-all translate-x-4 group-hover/similar:-translate-x-2"
        >
          <ChevronRight size={24} />
        </button>
        <div
