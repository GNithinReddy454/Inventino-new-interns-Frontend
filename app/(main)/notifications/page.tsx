...
import {
  ...
  AlertCircle, Search, CreditCard, RotateCcw, Users,
} from "lucide-react";
...
function groupByDate(notifications: Notification[]) {
  ...
  notifications.forEach((n) => {
    ...
    if (d.toDateString() === today.toDateString()) {
      label = `TODAY — ${today.toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }).toUpperCase()}`;
    } else if (d.toDateString() === yesterday.toDateString()) {
      label = `YESTERDAY — ${yesterday.toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }).toUpperCase()}`;
    } else {
      label = d.toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }).toUpperCase();
    }
          ...
          <div>
            ...
            {totalUnread > 0 && (
              <p className="text-sm text-gray-400 mt-1">
                {totalUnread} unread notification{totalUnread !== 1 ? "s" : ""} across all categories
              </p>
            )}
          </div>
            ...
            {TABS.map(tab => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 pb-3 pt-4 text-[13px] font-semibold whitespace-nowrap transition-all shrink-0 border-b-2 -mb-[2px] ${
                    active
                ...
                >
                  ...
                  {count > 0 && (
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ${
                      active ? "bg-[#D94F7A] text-white" : "bg-gray-100 text-gray-500"
                    }`}>
                      {count}
                    </span>
                  )}
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0 pr-6">
                            ...
                            <div className="flex items-center gap-2 mt-2 flex-wrap">
                              {s && (
                                <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-0.5 rounded-md border ${s.bg} ${s.text} ${s.border}`}>
                          ...
                          {/* Unread dot */}
                          {!notif.isRead && s && (
                            <span className={`absolute right-4 top-4 w-2.5 h-2.5 rounded-full shrink-0 ${s.dot}`} />
                          )}
