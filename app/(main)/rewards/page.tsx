...
export default function RewardsPage() {
    ...
    const rewards = [
        { id: 1, title: "10% OFF Coupon", points: 500, desc: "Get 10% discount on your next purchase", code: "GIFT10" },
        { id: 2, title: "Free Shipping", points: 300, desc: "Valid for all orders above ₹999", code: "SHIPFREE" },
        { id: 3, title: "₹500 Gift Voucher", points: 2000, desc: "Can be used on any jewelry collection", code: "VOUCH500" },
    ];

    const history = [
        { id: 1, action: "Purchased Diamond Ring", points: "+450", date: "Feb 12, 2026" },
        { id: 2, action: "Redeemed 10% Coupon", points: "-500", date: "Jan 28, 2026" },
        ...
        <div className="bg-[#fdf8f9] min-h-screen pt-4 md:pt-8 pb-20 font-sans">
            <div className="max-w-4xl mx-auto px-4">
                ...
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <h1 className="text-2xl font-serif font-bold text-[#D94F7A]">My Rewards</h1>
                </div>
                ...
                <div className="bg-gradient-to-br from-[#D94F7A] to-[#C0426A] rounded-[2.5rem] p-8 text-white shadow-xl shadow-pink-100 mb-8 relative overflow-hidden">
                    <div className="relative z-10">
                        <div className="flex justify-between items-start">
                            <div>
                                <p className="text-pink-100 text-sm font-bold uppercase tracking-wider mb-1">Available Points</p>
                        ...
                        <div className="mt-8 pt-6 border-t border-white/20">
                            <div className="flex justify-between items-center mb-2">
                                <p className="text-xs font-bold text-pink-100">750 points until Platinum</p>
                ...
                {activeTab === "earn" ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        {rewards.map((reward) => (
                            <div key={reward.id} className="bg-white p-6 rounded-[2rem] border border-pink-50 shadow-sm group hover:border-pink-200 transition-all">
                        ...
                        <div className="md:col-span-2 mt-4 bg-pink-50/50 rounded-[2rem] p-8 border border-dashed border-pink-200 flex flex-col md:flex-row items-center gap-6">
                            ...
                            <div className="flex-1 text-center md:text-left">
                                <h3 className="text-lg font-bold text-gray-900 mb-1">Refer & Earn More!</h3>
                                <p className="text-sm text-gray-600">Invite your friends and earn 500 points for every successful purchase they make.</p>
                            </div>
                            <Button variant="outline" className="border-pink-200 text-pink-600 hover:bg-white h-12 px-8 rounded-xl font-bold">
                                Share Link
                            </Button>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-[2rem] border border-pink-50 overflow-hidden shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-500">
                        {history.map((item, idx) => (
                            <div key={item.id} className={`flex items-center justify-between p-6 ${idx !== history.length - 1 ? 'border-b border-pink-50' : ''}`}>
                                <div className="flex items-center gap-4">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${item.points.startsWith('+') ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                                        {item.points.startsWith('+') ? <Zap size={18} /> : <ShoppingBag size={18} />}
                                    </div>
                                    <div>
                                ...
                                <p className={`font-black ${item.points.startsWith('+') ? 'text-emerald-500' : 'text-red-500'}`}>
                                    {item.points}
                                </p>
                            </div>
                        ))}
