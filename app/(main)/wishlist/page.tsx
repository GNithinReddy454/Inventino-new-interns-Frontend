...
import {
  ...
  CheckCircle2, X, Share2, ShoppingBag,
} from "lucide-react";
  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => {
        ...
        return (
          <svg key={star} width="10" height="10" viewBox="0 0 24 24" className="flex-shrink-0">
            <defs>
            ...
            <polygon
              ...
              fill={filled ? "#E8456A" : partial ? `url(#wl-star-${star}-${Math.round(rating * 10)})` : "#e5e7eb"}
            />
          </svg>
        );
      })}
...
function getBadgeInfo(badge: any): { text: string; color: string } | null {
  ...
  const text = typeof badge === "string" ? badge : badge?.text ? String(badge.text) : null;
  if (!text) return null;
  const upper = text.toUpperCase();
  if (upper === "BESTSELLER" || upper === "BEST SELLER") return { text, color: "bg-yellow-400" };
...
export default function WishlistPage() {
  ...
  const handleAddAllToCart = () => {
    ...
    selectedIds.forEach((id) => {
      ...
      if (item) { addToCart(item as any, 1); handleSaved(item); count++; }
    });
  ...
  const allSelected = savedItems.length > 0 && selectedIds.length === savedItems.length;
          ...
          <div className="flex items-center gap-3 mb-4 sm:mb-5">
            ...
            <label className="inline-flex items-center gap-2 cursor-pointer bg-white border border-[#F3D6EE] rounded-xl px-3 py-2 shadow-sm hover:border-[#E8456A] transition-colors">
              ...
              <div className="w-4 h-4 rounded-full border-2 border-[#F3D6EE] bg-white peer-checked:border-[#E8456A] peer-checked:bg-[#E8456A] transition-all flex items-center justify-center flex-shrink-0">
                <svg className={`w-2 h-2 text-white ${allSelected ? "opacity-100" : "opacity-0"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3.5}>
        ...
        {savedItems.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-2.5 sm:gap-4 md:gap-5">
            {savedItems.map((item: any) => {
              ...
              const rating = typeof item.rating === "number" ? item.rating : 4.7;
                  ...
                  <div className="relative aspect-square overflow-hidden bg-gray-50">
                    ...
                    <div className="absolute top-2 left-2 right-2 flex items-start justify-between z-10 pointer-events-none">
                      ...
                      <div className="pointer-events-none">
                        {badge ? (
                          <span className={`text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-sm shadow-sm ${badge.color}`}>
                            {badge.text}
                          </span>
                        ) : (
