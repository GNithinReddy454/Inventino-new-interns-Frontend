  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const featured = all.filter((p) => p.trendy).slice(0, 8);
      ...
      <div className="max-w-[1400px] w-full mx-auto">
        ...
        {loading && (
          <div className="flex gap-4 sm:gap-6 overflow-hidden">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className="flex-shrink-0 h-80 bg-pink-200 rounded-2xl animate-pulse
        ...
        {!loading && !error && products.length === 0 && (
          <p className="text-center text-gray-400 text-sm py-8">No featured products found.</p>
        )}
