...
const sanitizeProductForStore = (payload: any) => {
  return {
    ...payload,
    variants: Array.isArray(payload?.variants)
      ? payload.variants.map((variant: any) => ({
          id: variant?.id || "",
          color: variant?.color || "",
          expanded: Boolean(variant?.expanded),
          sizes: Array.isArray(variant?.sizes)
            ? variant.sizes.map((sizeItem: any) => ({
                size: sizeItem?.size || "",
...
const adminSlice = createSlice({
  ...
  reducers: {
    ...
    addProduct: (state, action: PayloadAction<any>) => {
      const safeProduct = sanitizeProductForStore(action.payload);
      state.localAddedProducts.unshift(safeProduct);
    },
    ...
    deleteProduct: (state, action: PayloadAction<string>) => {
      ...
      state.localAddedProducts = state.localAddedProducts.filter(
    ...
    // Optional helper if you want to clear old bad local products once
    clearLocalAddedProducts: (state) => {
      state.localAddedProducts = [];
    },
  },
});
